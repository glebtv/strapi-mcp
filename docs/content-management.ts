  ...
  return [
    {
      ...
      description: 'Lists all available content types in the Strapi instance (returns minimal info: uid, apiID, pluralApiId)',
      ...
      execute: async (args) => {
        ...
        let contentTypes = initData.contentTypes || [];
        
        // Filter by kind
        const kind = args.kind || 'user';
        if (kind !== 'all') {
          contentTypes = contentTypes.filter((ct: any) => {
            // System/plugin content types usually start with 'plugin::' or 'admin::'
        ...
        // Return minimal object to reduce token usage
        // Return as array directly, not as JSON string to avoid double escaping
        return contentTypes.map((ct: any) => ({
          uid: ct.uid,
          apiID: ct.apiID,
          pluralApiId: ct.pluralApiId
        }));
      }
    },
    {
      name: 'find_content_type',
      description: 'Find content types by searching uid, apiID, or pluralApiId. Returns full schema for matching content types.',
      inputSchema: z.object({
        filter: z.string().describe('Search filter - matches against uid, apiID, or pluralApiId (case-insensitive substring match)')
      }),
      execute: async (args) => {
        const initData = await client.listContentTypes();
        const contentTypes = initData.contentTypes || [];
        
        // Filter content types based on the search filter
        const filterLower = args.filter.toLowerCase();
        const matches = contentTypes.filter((ct: any) => {
          return (
        ...
        // If no matches found, return helpful error
        if (matches.length === 0) {
          return {
            error: `No content types found matching filter: "${args.filter}"`,
            available: contentTypes.map((ct: any) => ({
              uid: ct.uid,
              apiID: ct.apiID,
              pluralApiId: ct.pluralApiId
            }))
        ...
        // Return full schema for matched content types
        return {
