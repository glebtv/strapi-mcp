...
// Load the Strapi guide content
// Using relative path from compiled dist directory
const strapiGuideContent = `# Strapi MCP Working Guide
...
/**
 * Available prompts for the Strapi MCP server
 */
export const prompts: Record<string, Prompt> = {
  'strapi-tools': {
    name: 'strapi-tools',
    title: 'tools',
    description: 'Comprehensive guide for working with Strapi content management through MCP tools',
    arguments: []
  }
};
 ...
 */
export function getPromptContent(name: string, args: Record<string, unknown> = {}): string {
  switch (name) {
    case 'strapi-tools':
      return strapiGuideContent;
    default:
